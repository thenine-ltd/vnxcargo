...
if ( ! class_exists( 'YITH_YWBC_Backend' ) ) {
	...
	class YITH_YWBC_Backend {
		...
		public function init_hooks() {
			...
			add_action( 'wp_ajax_delete_barcode', array(
				$this,
		...
		public function create_barcode_callback() {
			...
			if ( ! in_array( 'yith_vendor', $user->roles ) && ! current_user_can( 'manage_woocommerce' ) && apply_filters( 'yith_barcode_role_check', true ) ) {
				return;
			}
		...
		public function create_order_barcode( $order_id, $protocol = '', $value = '' ) {
			...
			elseif ( $value_option == 'custom_field' ){
				...
				if ( $custom_field == 'order_url' && $protocol == 'QRcode' ){
		...
		public function on_new_product( $new_status, $old_status, $post ) {
			...
			$post_type_allowed = apply_filters('yith_barcodes_post_type_allowed',array('product'), $post );
		...
		public function create_product_barcode( $product_id, $protocol = '', $value = '' ) {
			...
			elseif ( $value_option == 'product_url' && $protocol == 'QRcode'){
		...
		public function show_barcode_generation_section( $post ) {
			if ( ( "shop_order" == $post->post_type ) ||
				...
				<div class="ywbc-barcode-generation">
					...
					if ( get_option ( 'ywbc_product_manual_barcode_product', 'no' ) == 'yes' && $post->post_type == 'product' || $post->post_type == 'product_variation' && apply_filters( 'ywbc_show_manual_barcode_section_in_product', true ) ) {
			...
			<div class="ywbc-generate-barcode">
				...
				<div>
					...
					<button class="button button-primary ywbc-delete-barcode" style="margin-top: 10px;"
					        data-id="<?php echo $obj_id; ?>"><?php esc_html_e( 'Delete', 'yith-woocommerce-barcodes' ); ?></button>
				</div>
			</div>
			<?php
		}
		...
		public function print_barcodes_selector_change() {
			...
			if ( $_POST['mode'] == 'include_variations' ){
