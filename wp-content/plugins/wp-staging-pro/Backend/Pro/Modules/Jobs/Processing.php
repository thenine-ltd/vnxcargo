...
class Processing extends Job
{
     ...
     * @var string
     */
    const FILTER_PUSH_EXCLUDED_DIRECTORIES = 'wpstg.push_excluded_directories';
    ...
    private function init()
    {
        ...
        // Add upload folder to list of excluded directories for push if staging uploads folder is actually a link
        $stagingUploadsFolder = $this->options->path . WPStaging::make(WpDefaultDirectories::class)->getRelativeUploadPath();
        if (is_link($stagingUploadsFolder)) {
            $excludedDirectories[] = rtrim($stagingUploadsFolder, '/\\');
        }
        ...
        $this->options->excludedDirectories = Hooks::applyFilters(self::FILTER_PUSH_EXCLUDED_DIRECTORIES, $this->options->excludedDirectories);
     ...
     * @return void
     */
    private function writePushLogHeader()
    {
