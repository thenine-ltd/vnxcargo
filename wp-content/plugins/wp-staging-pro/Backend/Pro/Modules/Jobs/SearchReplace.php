...
class SearchReplace extends JobExecutable
{
    ...
    protected function execute()
    {
        ...
        // Table is excluded
        $tablesExcluded = property_exists($this->options, 'tablesExcludedByFilter') ? $this->options->tablesExcludedByFilter : [];
        if (!empty($this->options->tables[$this->options->currentStep]) && in_array((string)$this->options->tables[$this->options->currentStep], $tablesExcluded)) {
    ...
    private function searchReplace($table, $args)
    {
        ...
        $filter[] = FinishBackupTask::OPTION_LAST_BACKUP;
        ...
        foreach ($data as $row) {
            ...
            foreach ($columns as $column) {
                $dataRow = isset($row[$column]) ? $row[$column] : '';

                // Skip column larger than 5MB
                $size = strlen((string)$dataRow);
                if ($size >= 5000000) {
                    continue;
                }
