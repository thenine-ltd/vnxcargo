...
class Upgrade
{
     ...
     * @return void
     */
    private function upgrade5_0_7()
    {
        // Early bail: Previous version is greater than 5.0.6
        if (version_compare($this->previousVersion, '5.0.6', '>')) {
            return;
        }
    ...
    /**
     * Upgrade method if user install WP Staging Pro initially and don't upgrade from previous version.
     * Activate email error report option and set the email address to the current install user.
     * @return void
     */
    private function upgrade5_0_5()
    {
        // Bail if user is upgrading from a previous version.
        if (!empty($this->previousVersion)) {
            return;
        }

        $user = wp_get_current_user();
        if (is_admin() && current_user_can('activate_plugins') && !empty($user->user_email)) {
            update_option(BackupScheduler::OPTION_BACKUP_SCHEDULE_REPORT_EMAIL, $user->user_email);
        }
     ...
     * @return void
     */
    private function upgrade4_0_6(bool $forceUpgrade = false)
    {
        $forceUpgrade = false;
     ...
     * @return string
     */
    private function getStagingPrefix(string $directory): string
    {
     ...
     * @return bool
     */
    private function setVersion(): bool
    {
     ...
     * @return bool
     */
    private function isInvalidVersionNumber(): bool
    {
    ...
    private function upgradeClonesBeta()
    {
        ...
        foreach ($this->clones as $key => &$value) {
            ...
            $existingClones[$value]['url']           = $urlsHelper->getHomeUrl() . "/" . $value;
