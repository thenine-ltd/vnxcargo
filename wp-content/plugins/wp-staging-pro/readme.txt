...
== Installation ==
1. Download the backup & clone plugin "wp-staging-pro.zip" from your account or the email we've sent to you.
2. Upload and install the backup plugin via the WordPress plugin backend wp-admin > plugins > add new > uploads
...
= 5.1.3 =
* Fix: There was a potential security vulnerability due to which a malicious user could eventually exploit and access the wp staging backup cache folder. This issue is now fixed. The security issue was found by Dmitrii Ignatyev from cleantalk.org. #2908
* Fix: Siteground backup related issue: file_put_contents() doesn't free up resources automatically immediately, which caused error 500 during backup extraction on SiteGround hosting (due to limited resources). #2868
...
* Fix: Copy drop-in file for Redis Object Cache when cloning live site #2239
...
* Enh: Add better logging for backup processing #2198
...
* Fix: During UPDATE and RESET for entire network clones default SUBSITES selected tables were not pre-selected #1992
