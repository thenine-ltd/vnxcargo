...
use WPStaging\Vendor\Aws\Endpoint\PartitionInterface;
class MultiRegionClient implements \WPStaging\Vendor\Aws\AwsClientInterface
{
    use AwsClientTrait;
    ...
    {
        $args = \array_intersect_key(\WPStaging\Vendor\Aws\ClientResolver::getDefaultArguments(), ['service' => \true, 'region' => \true]);
        ...
        }], 'partition' => ['type' => 'config', 'valid' => ['string', \WPStaging\Vendor\Aws\Endpoint\PartitionInterface::class], 'doc' => 'AWS partition to connect to. Valid partitions' . ' include "aws," "aws-cn," and "aws-us-gov." Used to' . ' restrict the scope of the mapRegions method.', 'default' => function (array $args) {
            $region = isset($args['region']) ? $args['region'] : '';
            return \WPStaging\Vendor\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartition($region, $args['service']);
        }, 'fn' => function ($value, array &$args) {
            if (\is_string($value)) {
                $value = \WPStaging\Vendor\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartitionByName($value);
            }
            if (!$value instanceof \WPStaging\Vendor\Aws\Endpoint\PartitionInterface) {
    ...
    public function __construct(array $args = [])
    {
        ...
        $this->handlerList = new \WPStaging\Vendor\Aws\HandlerList(function (\WPStaging\Vendor\Aws\CommandInterface $command) {
        ...
        $resolver = new \WPStaging\Vendor\Aws\ClientResolver($argDefinitions);
