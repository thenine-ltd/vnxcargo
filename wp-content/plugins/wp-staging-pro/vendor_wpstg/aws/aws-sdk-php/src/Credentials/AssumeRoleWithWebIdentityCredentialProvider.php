...
use WPStaging\Vendor\Aws\Sts\StsClient;
...
class AssumeRoleWithWebIdentityCredentialProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['client'])) {
            ...
            $this->client = new \WPStaging\Vendor\Aws\Sts\StsClient(['credentials' => \false, 'region' => $region, 'version' => 'latest']);
        }
    }
    /**
    ...
    {
        return \WPStaging\Vendor\GuzzleHttp\Promise\Coroutine::of(function () {
            $client = $this->client;
            $result = null;
            while ($result == null) {
                try {
                    ...
                    if (\false === $token) {
                        ...
                        if (!@\is_readable($this->tokenFile)) {
                            throw new \WPStaging\Vendor\Aws\Exception\CredentialsException("Unreadable tokenfile at location {$this->tokenFile}");
                        }
                    ...
                    if (empty($token)) {
                        ...
                        throw new \WPStaging\Vendor\Aws\Exception\CredentialsException("InvalidIdentityToken from file: {$this->tokenFile}");
                    }
                } catch (\Exception $exception) {
                    throw new \WPStaging\Vendor\Aws\Exception\CredentialsException("Error reading WebIdentityTokenFile from " . $this->tokenFile, 0, $exception);
                }
                ...
                try {
                ...
                } catch (\WPStaging\Vendor\Aws\Exception\AwsException $e) {
                    if ($e->getAwsErrorCode() == 'InvalidIdentityToken') {
                        if ($this->authenticationAttempts < $this->retries) {
                            ...
                            throw new \WPStaging\Vendor\Aws\Exception\CredentialsException("InvalidIdentityToken, retries exhausted");
                        }
                    } else {
                        throw new \WPStaging\Vendor\Aws\Exception\CredentialsException("Error assuming role from web identity credentials", 0, $e);
                    }
                } catch (\Exception $e) {
                    throw new \WPStaging\Vendor\Aws\Exception\CredentialsException("Error retrieving web identity credentials: " . $e->getMessage() . " (" . $e->getCode() . ")");
                }
