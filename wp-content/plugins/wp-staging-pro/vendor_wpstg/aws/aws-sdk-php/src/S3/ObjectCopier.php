...
use WPStaging\Vendor\Aws\Arn\S3\AccessPointArn;
use WPStaging\Vendor\Aws\Exception\MultipartUploadException;
 ...
 * when appropriate.
 */
class ObjectCopier implements \WPStaging\Vendor\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_MULTIPART_THRESHOLD = \WPStaging\Vendor\Aws\S3\MultipartUploader::PART_MAX_SIZE;
    private $client;
    private $source;
    private $destination;
    private $acl;
     ...
     */
    public function __construct(\WPStaging\Vendor\Aws\S3\S3ClientInterface $client, array $source, array $destination, $acl = 'private', array $options = [])
    {
    ...
    public function promise()
    {
        return \WPStaging\Vendor\GuzzleHttp\Promise\Coroutine::of(function () {
            ...
            if ($objectStats['ContentLength'] > $this->options['mup_threshold']) {
                $mup = new \WPStaging\Vendor\Aws\S3\MultipartCopy($this->client, $this->getSourcePath(), ['source_metadata' => $objectStats, 'acl' => $this->acl] + $this->destination + $this->options);
                (yield $mup->promise());
            } else {
    ...
    {
        if (\WPStaging\Vendor\Aws\Arn\ArnParser::isArn($this->source['Bucket'])) {
            try {
                new \WPStaging\Vendor\Aws\Arn\S3\AccessPointArn($this->source['Bucket']);
            } catch (\Exception $e) {
