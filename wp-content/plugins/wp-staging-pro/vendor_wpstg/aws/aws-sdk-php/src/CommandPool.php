...
use WPStaging\Vendor\GuzzleHttp\Promise\EachPromise;
/**
 ...
 */
class CommandPool implements \WPStaging\Vendor\GuzzleHttp\Promise\PromisorInterface
{
    /** @var EachPromise */
    private $each;
    /**
     ...
     */
    public function __construct(\WPStaging\Vendor\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        ...
        $mapFn = function ($commands) use($client, $before, $config) {
            foreach ($commands as $key => $command) {
                if (!$command instanceof \WPStaging\Vendor\Aws\CommandInterface) {
        ...
        $this->each = new \WPStaging\Vendor\GuzzleHttp\Promise\EachPromise($mapFn($commands), $config);
    }
    /**
     ...
     */
    public static function batch(\WPStaging\Vendor\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        $results = [];
