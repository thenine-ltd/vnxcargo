...
class TraceMiddleware
{
     ...
     *   "scrub_auth" is set to true.
     */
    public function __construct(array $config = [], \WPStaging\Vendor\Aws\Api\Service $service = null)
    {
    ...
    public function __invoke($step, $name)
    {
        ...
        return function (callable $next) use($step, $name) {
            return function (\WPStaging\Vendor\Aws\CommandInterface $command, \WPStaging\Vendor\Psr\Http\Message\RequestInterface $request = null) use($next, $step, $name) {
                ...
                return $next($command, $request)->then(function ($value) use($step, $name, $command, $start) {
                    ...
                    return new \WPStaging\Vendor\GuzzleHttp\Promise\RejectedPromise($reason);
                });
            };
        };
    }
    ...
    private function commandArray(\WPStaging\Vendor\Aws\CommandInterface $cmd)
    {
        return ['instance' => \spl_object_hash($cmd), 'name' => $cmd->getName(), 'params' => $this->getRedactedArray($cmd)];
    }
    private function requestArray(\WPStaging\Vendor\Psr\Http\Message\RequestInterface $request = null)
    {
    ...
    private function responseArray(\WPStaging\Vendor\Psr\Http\Message\ResponseInterface $response = null)
    {
    ...
    {
        return $value instanceof \WPStaging\Vendor\Aws\ResultInterface ? ['instance' => \spl_object_hash($value), 'data' => $value->toArray()] : $value;
    }
    private function exceptionArray($e)
    {
        ...
        if ($e instanceof \WPStaging\Vendor\Aws\Exception\AwsException) {
    ...
    private function streamStr(\WPStaging\Vendor\Psr\Http\Message\StreamInterface $body)
    {
        return $body->getSize() < $this->config['stream_size'] ? (string) $body : 'stream(size=' . $body->getSize() . ')';
    }
    private function createHttpDebug(\WPStaging\Vendor\Aws\CommandInterface $command)
    {
        if ($this->config['http'] && !isset($command['@http']['debug'])) {
            $command['@http']['debug'] = \fopen('php://temp', 'w+');
        }
    }
    private function flushHttpDebug(\WPStaging\Vendor\Aws\CommandInterface $command)
    {
     ...
     * @return array
     */
    private function getRedactedArray(\WPStaging\Vendor\Aws\CommandInterface $cmd)
    {
