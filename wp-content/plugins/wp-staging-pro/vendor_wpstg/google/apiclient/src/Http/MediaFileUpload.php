...
use WPStaging\Vendor\Google\Client;
...
class MediaFileUpload
{
     ...
     */
    public function __construct(\WPStaging\Vendor\Google\Client $client, \WPStaging\Vendor\Psr\Http\Message\RequestInterface $request, $mimeType, $data, $resumable = \false, $chunkSize = 0)
    {
        $this->client = $client;
    ...
    public function nextChunk($chunk = \false)
    {
        ...
        $request = new \WPStaging\Vendor\GuzzleHttp\Psr7\Request('PUT', $resumeUri, $headers, \WPStaging\Vendor\GuzzleHttp\Psr7\Utils::streamFor($chunk));
     ...
     *
     */
    private function makePutRequest(\WPStaging\Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        return \WPStaging\Vendor\Google\Http\REST::decodeHttpResponse($response, $this->request);
    }
    /**
    ...
    public function resume($resumeUri)
    {
        ...
        $httpRequest = new \WPStaging\Vendor\GuzzleHttp\Psr7\Request('PUT', $this->resumeUri, $headers);
    ...
    private function process()
    {
        ...
        $request = $request->withUri(\WPStaging\Vendor\GuzzleHttp\Psr7\Uri::withQueryValue($request->getUri(), 'uploadType', $uploadType));
        ...
        $request = $request->withBody(\WPStaging\Vendor\GuzzleHttp\Psr7\Utils::streamFor($postBody));
        if ($contentType) {
    ...
    private function fetchResumeUri()
    {
        ...
        throw new \WPStaging\Vendor\Google\Exception($error);
    }
    private function transformToUploadUrl()
    {
        ...
        $uri = \WPStaging\Vendor\GuzzleHttp\Psr7\Uri::fromParts($parts);
