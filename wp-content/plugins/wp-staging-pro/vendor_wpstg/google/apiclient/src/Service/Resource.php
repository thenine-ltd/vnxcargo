 ...
 */
namespace WPStaging\Vendor\Google\Service;

use WPStaging\Vendor\Google\Exception as GoogleException;
use WPStaging\Vendor\Google\Http\MediaFileUpload;
use WPStaging\Vendor\Google\Model;
use WPStaging\Vendor\Google\Utils\UriTemplate;
...
class Resource
{
    ...
    public function call($name, $arguments, $expectedClass = null)
    {
        if (!isset($this->methods[$name])) {
            ...
            throw new \WPStaging\Vendor\Google\Exception("Unknown function: " . "{$this->serviceName}->{$this->resourceName}->{$name}()");
        }
        ...
        if (isset($parameters['postBody'])) {
            if ($parameters['postBody'] instanceof \WPStaging\Vendor\Google\Model) {
        ...
        foreach ($parameters as $key => $val) {
            if ($key != 'postBody' && !isset($method['parameters'][$key])) {
                ...
                throw new \WPStaging\Vendor\Google\Exception("({$name}) unknown parameter: '{$key}'");
            }
        }
        foreach ($method['parameters'] as $paramName => $paramSpec) {
            if (isset($paramSpec['required']) && $paramSpec['required'] && !isset($parameters[$paramName])) {
                ...
                throw new \WPStaging\Vendor\Google\Exception("({$name}) missing required param: '{$paramName}'");
            }
        ...
        $request = new \WPStaging\Vendor\GuzzleHttp\Psr7\Request($method['httpMethod'], $url, $postBody ? ['content-type' => 'application/json'] : [], $postBody ? \json_encode($postBody) : '');
        // support uploads
        if (isset($parameters['data'])) {
            $mimeType = isset($parameters['mimeType']) ? $parameters['mimeType']['value'] : 'application/octet-stream';
            $data = $parameters['data']['value'];
            $upload = new \WPStaging\Vendor\Google\Http\MediaFileUpload($this->client, $request, $mimeType, $data);
    ...
    public function createRequestUri($restPath, $params)
    {
        ...
        if (\count($uriTemplateVars)) {
            $uriTemplateParser = new \WPStaging\Vendor\Google\Utils\UriTemplate();
