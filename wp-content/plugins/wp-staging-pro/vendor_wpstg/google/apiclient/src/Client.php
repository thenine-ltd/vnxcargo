 ...
 */
namespace WPStaging\Vendor\Google;

use BadMethodCallException;
use DomainException;
use WPStaging\Vendor\Google\AccessToken\Revoke;
use WPStaging\Vendor\Google\AccessToken\Verify;
use WPStaging\Vendor\Google\Auth\ApplicationDefaultCredentials;
use WPStaging\Vendor\Google\Auth\Cache\MemoryCacheItemPool;
use WPStaging\Vendor\Google\Auth\Credentials\ServiceAccountCredentials;
use WPStaging\Vendor\Google\Auth\Credentials\UserRefreshCredentials;
...
use WPStaging\Vendor\Google\Auth\OAuth2;
use WPStaging\Vendor\Google\AuthHandler\AuthHandlerFactory;
use WPStaging\Vendor\Google\Http\REST;
use WPStaging\Vendor\GuzzleHttp\Client as GuzzleClient;
use WPStaging\Vendor\GuzzleHttp\ClientInterface;
use WPStaging\Vendor\GuzzleHttp\Ring\Client\StreamHandler;
use InvalidArgumentException;
use LogicException;
use WPStaging\Vendor\Monolog\Handler\StreamHandler as MonologStreamHandler;
use WPStaging\Vendor\Monolog\Handler\SyslogHandler as MonologSyslogHandler;
...
class Client
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (!\is_null($this->config['credentials'])) {
            if ($this->config['credentials'] instanceof \WPStaging\Vendor\Google\Auth\CredentialsLoader) {
    ...
    public function fetchAccessTokenWithAuthCode($code)
    {
        ...
        $httpHandler = \WPStaging\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build($this->getHttpClient());
     ...
     * @return array access token
     */
    public function fetchAccessTokenWithAssertion(\WPStaging\Vendor\GuzzleHttp\ClientInterface $authHttp = null)
    {
        ...
        $httpHandler = \WPStaging\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build($authHttp);
     ...
     */
    public function authorize(\WPStaging\Vendor\GuzzleHttp\ClientInterface $http = null)
    {
    ...
    {
        $tokenRevoker = new \WPStaging\Vendor\Google\AccessToken\Revoke($this->getHttpClient());
    ...
    {
        $tokenVerifier = new \WPStaging\Vendor\Google\AccessToken\Verify($this->getHttpClient(), $this->getCache(), $this->config['jwt']);
        if (null === $idToken) {
     ...
     */
    public function execute(\WPStaging\Vendor\Psr\Http\Message\RequestInterface $request, $expectedClass = null)
    {
        ...
        $http = $this->authorize();
        return \WPStaging\Vendor\Google\Http\REST::execute($http, $request, $expectedClass, $this->config['retry'], $this->config['retry_map']);
    }
    /**
    ...
    {
        $auth = new \WPStaging\Vendor\Google\Auth\OAuth2(['clientId' => $this->getClientId(), 'clientSecret' => $this->getClientSecret(), 'authorizationUri' => self::OAUTH2_AUTH_URL, 'tokenCredentialUri' => self::OAUTH2_TOKEN_URI, 'redirectUri' => $this->getRedirectUri(), 'issuer' => $this->config['client_id'], 'signingKey' => $this->config['signing_key'], 'signingAlgorithm' => $this->config['signing_algorithm']]);
        return $auth;
    }
    /**
     ...
     */
    public function setCache(\WPStaging\Vendor\Psr\Cache\CacheItemPoolInterface $cache)
    {
        $this->cache = $cache;
    }
    /**
    ...
    {
        $logger = new \WPStaging\Vendor\Monolog\Logger('google-api-php-client');
        if ($this->isAppEngine()) {
            $handler = new \WPStaging\Vendor\Monolog\Handler\SyslogHandler('app', \LOG_USER, \WPStaging\Vendor\Monolog\Logger::NOTICE);
        } else {
            $handler = new \WPStaging\Vendor\Monolog\Handler\StreamHandler('php://stderr', \WPStaging\Vendor\Monolog\Logger::NOTICE);
        }
    ...
    {
        return new \WPStaging\Vendor\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
     * Set the Http Client object
     * @param ClientInterface $http
     */
    public function setHttpClient(\WPStaging\Vendor\GuzzleHttp\ClientInterface $http)
    {
        $this->http = $http;
    }
    /**
    ...
    {
        $guzzleVersion = null;
        if (\defined('\\WPStaging\\Vendor\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
            $guzzleVersion = \WPStaging\Vendor\GuzzleHttp\ClientInterface::MAJOR_VERSION;
        } elseif (\defined('\\WPStaging\\Vendor\\GuzzleHttp\\ClientInterface::VERSION')) {
            $guzzleVersion = (int) \substr(\WPStaging\Vendor\GuzzleHttp\ClientInterface::VERSION, 0, 1);
        }
        if (5 === $guzzleVersion) {
            $options = ['base_url' => $this->config['base_path'], 'defaults' => ['exceptions' => \false]];
            if ($this->isAppEngine()) {
                if (\class_exists(\WPStaging\Vendor\GuzzleHttp\Ring\Client\StreamHandler::class)) {
                    // set StreamHandler on AppEngine by default
                    $options['handler'] = new \WPStaging\Vendor\GuzzleHttp\Ring\Client\StreamHandler();
        ...
        return new \WPStaging\Vendor\GuzzleHttp\Client($options);
    }
    /**
    ...
    private function createApplicationDefaultCredentials()
    {
        ...
        if ($signingKey) {
            ...
            $credentials = \WPStaging\Vendor\Google\Auth\CredentialsLoader::makeCredentials($scopes, $serviceAccountCredentials);
        } else {
            ...
            $credentials = \WPStaging\Vendor\Google\Auth\ApplicationDefaultCredentials::getCredentials($scopes, null, $sub ? null : $this->config['cache_config'], $sub ? null : $this->getCache(), $this->config['quota_project']);
        }
        // for service account domain-wide authority (impersonating a user)
        // @see https://developers.google.com/identity/protocols/OAuth2ServiceAccount
        if ($sub) {
            if (!$credentials instanceof \WPStaging\Vendor\Google\Auth\Credentials\ServiceAccountCredentials) {
        ...
        if (!$credentials instanceof \WPStaging\Vendor\Google\Auth\FetchAuthTokenCache) {
            $credentials = new \WPStaging\Vendor\Google\Auth\FetchAuthTokenCache($credentials, $this->config['cache_config'], $this->getCache());
        }
        return $credentials;
    }
    protected function getAuthHandler()
    {
        ...
        return \WPStaging\Vendor\Google\AuthHandler\AuthHandlerFactory::build($this->getCache(), $this->config['cache_config']);
    }
    private function createUserRefreshCredentials($scope, $refreshToken)
    {
        $creds = \array_filter(['client_id' => $this->getClientId(), 'client_secret' => $this->getClientSecret(), 'refresh_token' => $refreshToken]);
        return new \WPStaging\Vendor\Google\Auth\Credentials\UserRefreshCredentials($scope, $creds);
    }
}
