...
<div class="ohio-widget portfolio-projects<?php echo esc_attr( $wrapper_classes ); ?> <?php if ( $is_slider ) echo 'vc_row'; ?>" id="<?php echo esc_attr( $wrapper_id ); ?>" <?php echo esc_attr( $animation_attrs); ?> data-ohio-portfolio-grid="true">
	...
	<?php if ( $show_projects_filter && !$is_slider ) : ?>
		...
		>
			<ul class="-unlist">
				...
				<li>
                    <a class="active<?php if ( $filter_layout == 'button' ) { echo esc_attr( ' tag -unlink'); } ?>" href="#all" data-isotope-filter="*" data-category-count="<?php echo str_pad( (string)$all_projects_count, 2, '0', STR_PAD_LEFT ); ?>">
				...
				<?php foreach ( $cat_ids as $cat_obj ) : ?>
					...
					?>
					<li>
						<a class="<?php if ( $filter_layout == 'button' ) { echo esc_attr( 'tag -unlink'); } ?>" href="#<?php echo esc_attr( $cat_obj->slug ); ?>" data-isotope-filter=".ohio-filter-project-<?php echo hash( 'md4', $cat_obj->slug, false ); ?>" data-category-count="<?php echo esc_attr( $cat_obj->count);?>">
	...
	<?php if ( !$is_slider ) : ?>
		...
		<div class="vc_row portfolio-grid<?php if ( preg_replace("/[^0-9.]/", '', $grid_items_gap ) == '0' ) { echo esc_html(' -nospace'); } ?>" data-isotope-grid="true" data-lazy-container="projects" data-projects-per-page="<?php echo esc_attr( $projects_in_block); ?>">
			...
			<?php
				...
				$ohio_project['excerpt_visible'] = $show_excerpt;
