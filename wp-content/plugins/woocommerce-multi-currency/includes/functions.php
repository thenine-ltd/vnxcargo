...
if ( ! function_exists( 'wmc_get_price' ) ) {
	function wmc_get_price( $price, $currency_code = false, $none_hook = false, $match_decimals = false ) {
		...
		$match_decimals = apply_filters( 'wmc_convert_price_matching_decimals', $match_decimals, $currency_code, $none_hook );
		/*Check currency*/
		...
		if ( $price ) {
			if ( $currency_code && isset( $selected_currencies[ $currency_code ] ) ) {
				...
				$price = $none_hook ? $price : apply_filters( 'wmc_get_price', $price, $currency_code );
			} else {
				...
				$price = $none_hook ? $price : apply_filters( 'wmc_get_price', $price, $current_currency );
			}
		}
