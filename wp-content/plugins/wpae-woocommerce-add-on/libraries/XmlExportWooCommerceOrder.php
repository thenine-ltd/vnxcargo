...
if (!class_exists('XmlExportWooCommerceOrder')) {
    final class XmlExportWooCommerceOrder
    {
        ...
        protected function prepare_export_data($record, $options, $elId, $preview)
        {
            ...
            $internalOrder = wc_get_order($orderId);
            // Ensure the order object includes the get_refunds method since order refund records do not.
            $internalRefunds = method_exists($internalOrder, 'get_refunds') ? $internalOrder->get_refunds() : [];

            $implode_delimiter = XmlExportEngine::$implode;

            if (empty($this->order_id) or $this->order_id != $orderId) {
                ...
                $all_order_items = $wpdb->get_results("SELECT * FROM {$table_prefix}woocommerce_order_items WHERE order_id = {$orderId}");
            ...
            if (!empty($options['cc_value'][$elId])) {
                ...
                switch ($options['cc_options'][$elId]) {
                    ...
                    case 'customer':
                        ...
                        if($this->isHPOSEnabled()) {
                            $metaName = 'get'. $options['cc_value'][$elId];

                            if($metaName === 'get_order_currency') {
                                $metaName = 'get_currency';
                            }

                            if($metaName === 'get_order_total') {
                                $metaName = 'get_total';
                            }

                            if(method_exists('WC_Order', $metaName)) {
                                $data[$options['cc_name'][$elId]] = (strpos($options['cc_value'][$elId], "_") === 0) ? $internalOrder->$metaName() : $record->{$elName};
                            } else {

                                if($elName === 'post_date') {
                                    $elName = 'date_created_gmt';
                                }
                                else if($elName === 'post_status') {
                                    $elName = 'status';
                                } else if($elName === 'post_excerpt') {
                                    $elName = 'customer_note';
                                }
                                if($elName != 'post_title') {
                                    $data[$options['cc_name'][$elId]] = (strpos($options['cc_value'][$elId], "_") === 0) ? $internalOrder->get_meta($options['cc_value'][$elId]) : $record->{$elName};
                                } else {
                                    $data[$options['cc_name'][$elId]] = '';
                                }
                            }
                        } else {
                            $data[$options['cc_name'][$elId]] = (strpos($options['cc_value'][$elId], "_") === 0) ? get_post_meta($orderId, $options['cc_value'][$elId], true) : $record->{$options['cc_value'][$elId]};
                        }
                        switch ($options['cc_value'][$elId]) {
                            ...
                            case 'post_date':
                                if ($this->isHPOSEnabled()) {
                                    $data[$options['cc_name'][$elId]] = prepare_date_field_value($options['cc_settings'][$elId], strtotime($internalOrder->get_date_created()), "Y-m-d H:i:s");

                                } else {
                                    $data[$options['cc_name'][$elId]] = prepare_date_field_value($options['cc_settings'][$elId], get_post_time('U', true, $orderId), "Y-m-d H:i:s");
                                }
                                break;
                            case '_completed_date':
                                if($this->isHPOSEnabled()) {
                                    $_completed_date = $internalOrder->get_date_completed();
                                    ...
                                    $_completed_date = get_post_meta($orderId, '_completed_date', true);
                                    $_completed_date_unix = empty($_completed_date) ? '' : strtotime($_completed_date);
                                    $data[$options['cc_name'][$elId]] = empty($_completed_date_unix) ? '' : prepare_date_field_value($options['cc_settings'][$elId], $_completed_date_unix, "Y-m-d H:i:s");
                                }
                                break;
                            case '_date_paid':
                                $_paid_date = $internalOrder->get_date_paid();
                                $_paid_date_unix = empty($_paid_date) ? '' : strtotime($_paid_date);

                                $data[$options['cc_name'][$elId]] = empty($_paid_date_unix) ? '' : prepare_date_field_value($options['cc_settings'][$elId], $_paid_date_unix, "Y-m-d H:i:s");

                                break;
                            case '_customer_user_email':
                                if($this->isHPOSEnabled()) {
                                    $data[$options['cc_name'][$elId]] = $record->billing_email;
                                } else {
                                    $customer_user_id = get_post_meta($orderId, '_customer_user', true);
                                    if ($customer_user_id) {
                                    ...
                                    if (empty($data[$options['cc_name'][$elId]])) $data[$options['cc_name'][$elId]] = get_post_meta($orderId, '_billing_email', true);
                                }
                                break;

                            case '_customer_user':
                                if($this->isHPOSEnabled()) {
                                    $data[$options['cc_name'][$elId]] = $internalOrder->get_customer_id();
                                } else {
                                    $customer_user_id = get_post_meta($orderId, '_customer_user', true);
                                }
                    ...
                    case 'refunds':
                        ...
                        if (!empty($this->order_refunds)) {
                            foreach ($this->order_refunds as $refund) {
                                ...
                                $internalRefund = wc_get_order($refundId);
                                ...
                                foreach ($options['ids'] as $subID => $subvalue) {
                                    if ($options['cc_type'][$subID] == 'woo_order' and $options['cc_options'][$subID] == 'refunds') {
                                        ...
                                        switch ($options['cc_value'][$subID]) {
                                            ...
                                            case 'refund_id':
                                                $refunds_data[$element_name] = pmxe_filter($refundId, $RefundsfieldSnipped);
                                                break;
                                            case 'refund_amount':
                                                if($this->isHPOSEnabled()) {
                                                    $refunds_data[$element_name] = pmxe_filter($refund->total_amount, $RefundsfieldSnipped);
                                                } else {
                                                    $refunds_data[$element_name] = pmxe_filter(get_post_meta($refundId, '_order_total', true), $RefundsfieldSnipped);
                                                }
                                                break;
                                            case 'refund_date':
                                                if($this->isHPOSEnabled()) {
                                                    $refunds_data[$element_name] = pmxe_filter(date("Ymd", strtotime($refund->date_created_gmt)), $RefundsfieldSnipped);
                                                } else {
                                                    $refunds_data[$element_name] = pmxe_filter(date("Ymd", get_post_time('U', true, $refund->ID)), $RefundsfieldSnipped);
                                                }
                                                break;
                                            case 'refund_reason':
                                                $refunds_data[$element_name] = pmxe_filter($internalRefund->get_reason(), $RefundsfieldSnipped);
                                                break;
                                            case 'refund_author_email':
                                                if($this->isHPOSEnabled()) {
                                                    $refundUserId = $wpdb->get_var("SELECT meta_value FROM {$wpdb->prefix}wc_orders_meta WHERE order_id = {$refundId} AND meta_key='_refunded_by'");
                                                } else {
                                                    $refundUserId = $refund->post_author;
                                                }

                                                $refund_author = get_userdata($refundUserId);
                        ...
                        switch ($options['cc_value'][$elId]) {
                            ...
                            case 'refund_order_shipping_amount':
                            ...
                            case 'refund_order_shipping_name':
                                $shipping_refunded_values = [];
                                foreach ($internalRefunds as $refund) {
                                    $shipping_refunded_values[] = $refund->get_shipping_method();
                                }
                                $data[$options['cc_name'][$elId]] = pmxe_filter(implode($implode_delimiter, $shipping_refunded_values), $fieldSnipped);

                                break;

                            case 'refund_order_shipping_tax_amount':

                                $shippingTaxRefunds = [];

                                foreach ($internalRefunds as $internalRefund) {

                                    $shippingTaxRefunds[] = $internalRefund->get_shipping_tax();


                                }

                                $data[$options['cc_name'][$elId]] = pmxe_filter(implode($implode_delimiter, $shippingTaxRefunds), $fieldSnipped);
                                break;

                            case 'refund_order_amount':

                                $refundData = [];
                                foreach ($internalRefunds as $internalRefund) {
                                    $refundOrderItems = [];
                                    foreach ($internalRefund->get_items() as $item) {
                                        $refundOrderItems[] = $item->get_subtotal();
                                    }


                                    $refundData[] = implode('#', $refundOrderItems);
                                }

                                $refundData = array_reverse($refundData);

                                $data[$options['cc_name'][$elId]] = pmxe_filter(implode($implode_delimiter, $refundData), $fieldSnipped);
                                break;

                            case 'refund_order_fee_amount':


                                $refundData = [];
                                foreach ($internalRefunds as $internalRefund) {
                                    $feeRefunds = [];
                                    $fees = $internalRefund->get_fees();
                                    foreach ($fees as $fee) {
                                        if ($fee->get_total() < 0) {
                                            $feeRefunds[] = $fee->get_total();
                                        } else {
                                            $feeRefunds[] = 0;
                                        }
                                    }

                                    $refundItem = implode('#', $feeRefunds);
                                    if ($refundItem === "") {
                                        $refundItem = "0";
                                    }
                            ...
                            case 'refund_order_item':
                                ...
                                foreach ($internalRefunds as $internalRefund) {
                                    $refundOrderItems = [];
                                    foreach ($internalRefund->get_items() as $item) {
                                        $refundOrderItems[] = $item->get_name();
                                    }
                                    $refundOrderItemData[] = implode('#', $refundOrderItems);
                                }

                                $data[$options['cc_name'][$elId]] = pmxe_filter(implode($implode_delimiter, $refundOrderItemData), $fieldSnipped);


                                break;
                            case 'refund_order_quantity':

                                $refundOrderQuantityData = [];

                                foreach ($internalRefunds as $internalRefund) {
                                    $refundOrderItems = [];
                                    foreach ($internalRefund->get_items() as $item) {
                                        $refundOrderItems[] = $item->get_quantity();
                                    }

                                    $refundOrderQuantityData[] = implode('#', $refundOrderItems);
                                }

                                $data[$options['cc_name'][$elId]] = pmxe_filter(implode($implode_delimiter, $refundOrderQuantityData), $fieldSnipped);
                                break;


                            case 'refund_order_tax_amount':

                                $refundOrderTaxAmoundData = [];

                                foreach ($internalRefunds as $internalRefund) {
                                    $refundOrderItems = [];
                                    foreach ($internalRefund->get_items() as $item) {
                                        $refundOrderItems[] = $item->get_total_tax();
                                    }

                                    $refundOrderTaxAmoundData[] = implode('#', $refundOrderItems);


                                }

                                $data[$options['cc_name'][$elId]] = pmxe_filter(implode($implode_delimiter, $refundOrderTaxAmoundData), $fieldSnipped);
                                break;


                            case 'refund_order_fee_tax_amount':

                                $refundOrderFeeTaxAmountData = [];

                                foreach ($internalRefunds as $internalRefund) {
                                    $feeTaxRefunds = [];
                                    $fees = $internalRefund->get_fees();
                                    foreach ($fees as $fee) {
                                        $refundAmount = $fee->get_total_tax();
                                        if ($refundAmount < 0) {
                                            $feeTaxRefunds[] = $refundAmount;
                                        } else {
                                            $feeTaxRefunds[] = 0;
                                        }
                                    }

                                    $refundOrderFeeTaxAmountData[] = implode('#', $feeTaxRefunds);


                                }

                                $data[$options['cc_name'][$elId]] = pmxe_filter(implode($implode_delimiter, $refundOrderFeeTaxAmountData), $fieldSnipped);

                                break;

                            case 'refund_item_tax_amount':

                                $refundOrderItemTaxAmountData = [];

                                foreach ($internalRefunds as $internalRefund) {
                                    $itemTaxRefunds = [];
                                    $items = $internalRefund->get_items();
                                    ...
                                    foreach ($items as $item) {
                                        ...
                                        $itemTaxRefunds[] = implode('@', $taxData);
                                    }

                                    $refundOrderItemTaxAmountData[] = implode('#', $itemTaxRefunds);


                                }

                                $data[$options['cc_name'][$elId]] = pmxe_filter(implode($implode_delimiter, $refundOrderItemTaxAmountData), $fieldSnipped);

                                break;

                            case 'refund_shipping_tax_amount':

                                $refundOrderShippingTaxAmountData = [];

                                foreach ($internalRefunds as $internalRefund) {
                                    $shippingTaxRefunds = [];
                                    $shippingMethods = $internalRefund->get_shipping_methods();
                                    foreach ($shippingMethods as $shippingMethod) {
                                        ...
                                        $shippingTaxRefunds[] = implode('@', $taxData);
                                    }

                                    $refundOrderShippingTaxAmountData[] = implode('#', $shippingTaxRefunds);


                                }

                                $data[$options['cc_name'][$elId]] = pmxe_filter(implode($implode_delimiter, $refundOrderShippingTaxAmountData), $fieldSnipped);

                                break;

                            case 'refund_fee_tax_amount':

                                $refundOrderFeeTaxAmountData = [];

                                foreach ($internalRefunds as $internalRefund) {
                                    $feeTaxRefunds = [];
                                    $shippingMethods = $internalRefund->get_fees();
                                    foreach ($shippingMethods as $shippingMethod) {
                                        $taxes = $shippingMethod->get_taxes();
                                        $taxData = [];
                                        foreach ($taxes['total'] as $tax) {
                                    ...
                                    if ($options['cc_type'][$subID] == 'woo_order' and $options['cc_options'][$subID] == 'items') {
                                        ...
                                        switch ($options['cc_value'][$subID]) {
                                            ...
                                            case '_line_subtotal_tax':
                                                ...
                                                $orderItems = $internalOrder->get_items();

                                                $fieldValue = '';
                                                foreach($orderItems as $item) {

                                                    if($item->get_id() == $order_item->order_item_id) {
                                                        $taxes = $item->get_taxes();
                                                        $fieldValue = implode('#', $taxes['total']);
                                                    }
                                                }

                                                $item_data[$element_name] = pmxe_filter($fieldValue, $ItemsfieldSnipped);

                                                break;
                                            case '_line_subtotal_tax_name':


                                                $taxes = $internalOrder->get_taxes();
                                                $taxNames = [];
                                                foreach ($taxes as $tax) {
                                                    $taxNames[] = $tax->get_label();

                                                }

                                                $fieldValue = implode('#', $taxNames);
                    ...
                    case 'taxes':
                        ...
                        if ($options['cc_value'][$elId] == '_order_tax') {
                            ...
                            if($this->isHPOSEnabled()) {
                                $_order_tax = $internalOrder->get_total_tax();

                                $_order_tax_total = $_order_tax;


                            } else {
                                $_order_shipping_tax = get_post_meta($orderId, '_order_shipping_tax', true);
                                $_order_tax = get_post_meta($orderId, '_order_tax', true);
                        ...
                        if ($options['cc_value'][$elId] === '_order_shipping_tax_code') {
                            $shippingTaxCodeData = [];

                            $shipping_methods = $internalOrder->get_shipping_methods();

                            $taxes = $internalOrder->get_taxes();
                            foreach ($shipping_methods as $shipping_method) {
                                $taxRates = [];
                                foreach ($taxes as $tax) {
                                    $taxRates[] = $tax->get_label();

                                }

                                $shippingTaxCodeData[] = implode('#', $taxRates);
                            }


                            $data[$options['cc_name'][$elId]] = implode($implode_delimiter, $shippingTaxCodeData);
                        }

                        if ($options['cc_value'][$elId] === '_order_fee_tax_code') {
                            $shippingTaxCodeData = [];

                            $fees = $internalOrder->get_fees();
                        ...
                        if ($options['cc_value'][$elId] === '_order_shipping_tax_amount') {
                            $shippingTaxAmountData = [];
                            ...
                            foreach ($shipping_methods as $shipping_method) {
                                ...
                                $shippingTaxAmountData[] = implode('#', $taxRates);
                            }


                            $data[$options['cc_name'][$elId]] = implode($implode_delimiter, $shippingTaxAmountData);
                        }

                        if ($options['cc_value'][$elId] === '_order_fee_tax_amount') {
                            $feeTaxAmountData = [];
                            ...
                            foreach ($fees as $fee) {
                                ...
                                $feeTaxAmountData[] = implode('#', $taxRates);
                            }


                            $data[$options['cc_name'][$elId]] = implode($implode_delimiter, $feeTaxAmountData);
                        }

                        if ($options['cc_value'][$elId] === '_order_shipping_name') {
                            $shippingNameData = [];
                            ...
                            foreach ($shipping_methods as $shipping_method) {
                                $taxRates = [];
                                $taxRates[] = $shipping_method->get_name();



                                $shippingNameData[] = implode('#', $taxRates);
                            }

                            $data[$options['cc_name'][$elId]] = implode($implode_delimiter, $shippingNameData);
                        }

                        if ($options['cc_value'][$elId] === '_order_shipping_id') {
                            $shippingNameData = [];

                            $shipping_methods = $internalOrder->get_shipping_methods();
                            foreach ($shipping_methods as $shipping_method) {
                                $taxRates = [];
                                $taxRates[] = $shipping_method->get_method_id();
                    ...
                    case 'fees':
                        ...
                        if ($options['cc_value'][$elId] == '_cart_discount') {
                            ...
                            $_cart_discount = $internalOrder->get_discount_total();

                            $data[$options['cc_name'][$elId]] = pmxe_filter((float)$_cart_discount * (-1), $fieldSnipped);
                        }

                        if ($options['cc_value'][$elId] === '__fee_tax_rate_name') {
                            $taxRateNameData = [];
                            ...
                            foreach ($fees as $fee) {
                                $taxRates = [];
                                foreach ($taxes as $tax) {
                                    $taxRates[] = $tax->get_rate_code();

                                }

                                $taxRateNameData[] = implode('#', $taxRates);
                            }


                            $data[$options['cc_name'][$elId]] = implode($implode_delimiter, $taxRateNameData);
                        }

                        if ($options['cc_value'][$elId] === '__fee_tax_rate_amount') {
                            $taxRateAmountData = [];
                            ...
                            foreach ($fees as $fee) {
                                ...
                                $taxRateAmountData[] = implode('#', $taxRates);
                            }


                            $data[$options['cc_name'][$elId]] = implode($implode_delimiter, $taxRateAmountData);
                        }
        ...
        public function export_xml(& $xmlWriter, $record, $options, $elId, $preview)
        {
            ...
            foreach ($data_to_export as $key => $data) {
                ...
                if (in_array($key, array('items', 'taxes', 'shipping', 'coupons', 'surcharge', 'refunds'))) {
                        ...
                        foreach ($data as $item) {
                            if (!empty($item)) {
                                ...
                                foreach ($item as $item_key => $item_value) {
                                    ...
                                    $element_name = preg_replace($charRegEx, '', preg_replace($startCharRegEx, '', preg_replace("%#\d*%", "", $item_key)));

                                    $element_name = empty($element_name) ? 'untitled_' . $elId : $element_name;
                    ...
                    $xmlWriter = apply_filters('wp_all_export_add_before_element', $xmlWriter, $element_name, XmlExportEngine::$exportID, $orderId);

                    $xmlWriter->beginElement($element_name_ns, $element_name, null);
                    $xmlWriter->writeData($data, $element_name);
                    $xmlWriter->closeElement();

                    $xmlWriter = apply_filters('wp_all_export_add_after_element', $xmlWriter, $element_name, XmlExportEngine::$exportID, $orderId);
                }
            }
        }
                                    ...
                                    <li class="pmxe_<?php echo esc_attr($slug); ?>  <?php if ($is_auto_field) echo esc_html('wp_all_export_auto_generate'); ?>">
                                        <div class="custom_column" rel="<?php echo intval($i + 1); ?>">
                                            ...
                                            <input type="hidden" name="cc_label[]"
                                                   value="<?php echo (is_array($field)) ? esc_attr($field['label']) : esc_attr($cur_meta_key); ?>"/>
                                            ...
                                            <input type="hidden" name="cc_options[]"
                                                   value="<?php echo (is_array($field)) ? esc_attr($field['options']) : esc_attr($slug); ?>"/>
                                            <input type="hidden" name="cc_type[]"
                                                   value="<?php echo (is_array($field)) ? esc_attr($field['type']) : esc_attr('woo_order'); ?>"/>
                                            ...
                                            <input type="hidden" name="cc_combine_multiple_fields[]"
                                                   value="<?php echo (is_array($field) && isset($field['cc_combine_multiple_fields'])) ? esc_attr($field['cc_combine_multiple_fields']) : ''; ?>"/>
                                            <input type="hidden" name="cc_combine_multiple_fields_value[]"
                                                   value="<?php echo (is_array($field) && isset($field['cc_combine_multiple_fields_value'])) ? esc_attr($field['cc_combine_multiple_fields_value']) : ''; ?>"/>
                            ...
                            if (!empty($section['additional'])) {
                                foreach ($section['additional'] as $sub_slug => $sub_section) {
                                    ...
                                    <li class="available_sub_section">
                                        <p class="wpae-available-fields-group"><?php echo esc_html($sub_section['title']); ?>
                                                ...
                                                foreach ($sub_section['meta'] as $field) {
                                                    ...
                                                    <li class="pmxe_<?php echo esc_attr($slug); ?>_<?php echo esc_attr($sub_slug); ?>">
                                                        <div class="custom_column" rel="<?php echo intval($i + 1); ?>">
                                                            ...
                                                            <input type="hidden" name="ids[]" value="1"/>
                                                            <input type="hidden" name="cc_label[]"
                                                                   value="item_data__<?php echo (is_array($field)) ? esc_attr($field['label']) : esc_attr($field); ?>"/>
                                                            <input type="hidden" name="cc_php[]" value=""/>
                                                            <input type="hidden" name="cc_code[]" value=""/>
                                                            <input type="hidden" name="cc_sql[]" value=""/>
        ...
        public function render_new_field()
        {
            ...
            <select class="wp-all-export-chosen-select" name="column_value_type" style="width:350px;">
                ...
                if (!$disable_acf && XMLExportEngine::get_addons_service()->isAcfAddonActive()) {
            ...
            $sections = array(
                ...
                'refunds' => array(
                    ...
                    'meta' => array(
                        ...
                        'refund_order_item' => __('Refund Order Item', 'wp_all_export_plugin'),
                        'refund_order_quantity' => __('Refund Order Quantity', 'wp_all_export_plugin'),
                        'refund_order_amount' => __('Refund Order Amount', 'wp_all_export_plugin'),
                        'refund_order_tax_amount' => __('Refund Order Tax Amount', 'wp_all_export_plugin'),
                        'refund_order_shipping_amount' => __('Refund Order Shipping Amount', 'wp_all_export_plugin'),
                        'refund_order_shipping_name' => __('Refund Order Shipping Name', 'wp_all_export_plugin'),
                        'refund_order_shipping_tax_amount' => __('Refund Order Shipping Tax Amount', 'wp_all_export_plugin'),
                        'refund_order_fee_amount' => __('Refund Order Fee Amount', 'wp_all_export_plugin'),
                        'refund_order_fee_name' => __('Refund Order Fee Name', 'wp_all_export_plugin'),
                        'refund_order_fee_tax_amount' => __('Refund Order Fee Tax Amount', 'wp_all_export_plugin'),


                        'refund_item_tax_amount' => __('Refund Item Tax Amount', 'wp_all_export_plugin'),
                        'refund_shipping_tax_amount' => __('Refund Shipping Tax Amount', 'wp_all_export_plugin'),
                        'refund_fee_tax_amount' => __('Refund Fee Tax Amount', 'wp_all_export_plugin'),
        ...
        public function available_order_data()
        {
            $data = array(
                ...
                '_date_paid' => __('Date Paid', 'wp_all_export_plugin')
            );
        ...
        public function available_order_default_product_data()
        {
            ...
            $data = array(
                ...
                '_line_subtotal_tax' => __('Item Tax Amount', 'wp_all_export_plugin'),
                '_line_subtotal_tax_name' => __('Item Tax Name', 'wp_all_export_plugin'),
        ...
        public function available_order_taxes_data()
        {
            ...
            $data = array(
                ...
                '_order_shipping_tax_code' => __('Shipping Tax Code', 'wp_all_export_plugin'),
                '_order_shipping_tax_amount' => __('Shipping Tax Amount', 'wp_all_export_plugin'),
                '_order_shipping_name' => __('Shipping Name', 'wp_all_export_plugin'),
                '_order_shipping_id' => __('Shipping ID', 'wp_all_export_plugin'),
                '_order_fee_tax_code' => __('Fee Tax Code', 'wp_all_export_plugin'),
                '_order_fee_tax_amount' => __('Fee Tax Amount', 'wp_all_export_plugin'),
        ...
        public function available_shipping_information_data()
        {
            ...
            $keys = array(
                ...
                '_shipping_postcode', '_shipping_country', '_shipping_state', '_shipping_phone'
            );
        ...
        public static function prepare_import_template($exportOptions, &$templateOptions, $element_name, $ID)
        {
            ...
            switch ($element_type) {
                ...
                case '_date_paid':
                    $templateOptions['is_update_dates'] = 1;
                    $templateOptions['pmwi_order']['payment_date'] = '{' . $element_name . '[1]}';
                    break;
                ...
                case '__product_sku':
                    ...
                    if ($is_xml_template) {
                        ...
                        $templateOptions['pmwi_order']['product_items'][0]['sku'] = '{' . $element_name . '[1]}';
                    } else {
                        ...
                        if ($options['order_item_per_row'] or $options['xml_template_type'] == 'custom') {
                            ...
                            $templateOptions['pmwi_order']['product_items'][0]['sku'] = "[str_replace(\",\",\"|\",{./*[starts-with(local-name(), '$element_name')]})]";
                        }
                    }
                    break;

                case '__product_title':
                    $templateOptions['pmwi_order']['product_items'][0]['unique_key'] = "[str_replace(\",\",\"|\",{./*[starts-with(local-name(), '$element_name')]})]";
                    break;
                case 'refund_order_shipping_amount' :

                    $templateOptions['pmwi_order']['refunds'][0]['shipping_refund_amount'] = '{' . $element_name . '[1]}';
                    break;
                case 'refund_order_shipping_name':
                    $templateOptions['pmwi_order']['refunds'][0]['shipping_name'] = '{' . $element_name . '[1]}';
                    break;
                case '_line_subtotal':

                    if ($is_xml_template) {
                        $templateOptions['pmwi_order']['product_items'][0]['price_per_unit'] = '{' . $element_name . '[1]}';
                    } else {
                        if ($options['order_item_per_row'] or $options['xml_template_type'] == 'custom') {
                            $templateOptions['pmwi_order']['product_items'][0]['price_per_unit'] = '{' . $element_name . '[1]}';
                        } else {
                            $templateOptions['pmwi_order']['product_items'][0]['price_per_unit'] = "[str_replace(\",\",\"|\",{./*[starts-with(local-name(), '$element_name')]})]";
                        }
                    }
                ...
                case '_line_subtotal_tax':
                    ...
                    if ($is_xml_template) {
                        $templateOptions['pmwi_order']['product_items'][0]['tax_rates'][0]['amount_per_unit'] = '{' . $element_name . '[1]}';
                    } else {
                        if ($options['order_item_per_row'] or $options['xml_template_type'] == 'custom') {
                            $templateOptions['pmwi_order']['product_items'][0]['tax_rates'][0]['amount_per_unit'] = '{' . $element_name . '[1]}';
                        } else {
                            $templateOptions['pmwi_order']['product_items'][0]['tax_rates'][0]['amount_per_unit'] = "[str_replace(\",\",\"|\",{./*[starts-with(local-name(), '$element_name')]})]";
                        }
                    }
                    break;

                case '_order_fee_tax_amount':
                    $templateOptions['pmwi_order']['fees'][0]['tax_rates'][0]['amount_per_unit'] = '{' . $element_name . '[1]}';
                    break;
                case '_order_tax':
                    $templateOptions['pmwi_order']['order_total_tax_amount'] = '{' . $element_name . '[1]}';
                    break;

                case '_qty':

                    if ($is_xml_template) {
                        $templateOptions['pmwi_order']['product_items'][0]['qty'] = '{' . $element_name . '[1]}';
                    } else {
                        $templateOptions['pmwi_order']['products_repeater_mode_separator'] = $implode_delimiter;
                        ...
                        if ($options['order_item_per_row'] or $options['xml_template_type'] == 'custom') {
                            ...
                            $templateOptions['pmwi_order']['product_items'][0]['qty'] = "[str_replace(\",\",\"|\",{./*[starts-with(local-name(), '$element_name')]})]";
                        }
                    }
                    break;
                ...
                case 'fee_line_total':
                    ...
                    if ($is_xml_template) {
                        ...
                        $templateOptions['pmwi_order']['refunds'][0]['fee_name'] = '{feename[1]}';
                ...
                case 'discount_amount':
                    ...
                    if ($is_xml_template) {
                        ...
                        $templateOptions['pmwi_order']['coupons'][0]['amount'] = '[str_replace("-","",{discountamount[1]})]';
                    } else {
                ...
                case '_order_total':
                    ...
                    $templateOptions['pmwi_order']['order_total_amount'] = "{ " . $element_name . "[1]}";
                    break;


                // Order Refunds
                case 'refund_amount':
                    $templateOptions['pmwi_order']['order_refund_type'] = 'partial';
                    ...
                    if ($is_xml_template) {
                        ...
                        $templateOptions['pmwi_order']['refunds'][0]['partial_amount'] = '{' . $element_name . '[1]}';
                    }
                    break;
                case 'refund_reason':
                    $templateOptions['pmwi_order']['order_refund_type'] = 'partial';
                    $templateOptions['pmwi_order']['is_update_refunds'] = 1;
                    $templateOptions['pmwi_order']['refunds'][0]['partial_reason'] = '{' . $element_name . '[1]}';
                    if ($is_xml_template) {
                        $templateOptions['pmwi_order']['order_' . $element_type] = '{OrderRefunds[1]/Refund/' . $element_name . '[1]}';
                    } else {
                ...
                case 'refund_date':
                    $templateOptions['pmwi_order']['order_refund_type'] = 'partial';
                    $templateOptions['pmwi_order']['is_update_refunds'] = 1;
                    $templateOptions['pmwi_order']['refunds'][0]['partial_date'] = '{' . $element_name . '[1]}';
                    if ($is_xml_template) {
                        $templateOptions['pmwi_order']['order_' . $element_type] = '{OrderRefunds[1]/Refund/' . $element_name . '[1]}';
                    } else {
                        $templateOptions['pmwi_order']['order_' . $element_type] = '{' . $element_name . '[1]}';
                ...
                case 'refund_author_email':
                    ...
                    break;
                case 'refund_order_item':
                    $templateOptions['pmwi_order']['refunds'][0]['order_item_name'] = '{' . $element_name . '[1]}';
                    break;
                case 'refund_order_quantity':
                    $templateOptions['pmwi_order']['refunds'][0]['item_refund_quantity'] = '{' . $element_name . '[1]}';
                    break;
                case 'refund_order_amount':
                    $templateOptions['pmwi_order']['refunds'][0]['item_refund_amount'] = '{' . $element_name . '[1]}';
                    break;
                case 'refund_item_tax_amount':
                    $templateOptions['pmwi_order']['refunds'][0]['item_refund_tax_amount'] = '{' . $element_name . '[1]}';
                    break;
                case 'refund_shipping_tax_amount':
                    $templateOptions['pmwi_order']['refunds'][0]['shipping_refund_tax_amount'] = '{' . $element_name . '[1]}';
                    break;
                case 'refund_order_fee_amount':
                    $templateOptions['pmwi_order']['refunds'][0]['fee_refund_amount'] = '{' . $element_name . '[1]}';
                    break;
                case 'refund_order_fee_name':
                    $templateOptions['pmwi_order']['refunds'][0]['fee_name'] = '{' . $element_name . '[1]}';
                    break;
                case 'refund_fee_tax_amount':
                    $templateOptions['pmwi_order']['refunds'][0]['fee_refund_tax_amount'] = '{' . $element_name . '[1]}';
                    break;
                case '_order_shipping_name':
                    $templateOptions['pmwi_order']['shipping'][0]['name'] = '{' . $element_name . '[1]}';
                    break;
                case '_order_shipping_tax_code':
                    $templateOptions['pmwi_order']['shipping'][0]['tax_rates'][0]['code'] = '{' . $element_name . '[1]}';
                    break;
                case '_order_shipping_tax_amount':
                    $templateOptions['pmwi_order']['shipping'][0]['tax_rates'][0]['amount_per_unit'] = '{' . $element_name . '[1]}';
                    break;
                case '_fee_amount':
                    $templateOptions['pmwi_order']['fees'][0]['amount'] = '{' . $element_name . '[1]}';
                    break;
                case '_line_subtotal_tax_name':
                    $templateOptions['pmwi_order']['product_items'][0]['tax_rates'][0]['code'] = "[str_replace(\",\",\"|\",{./*[starts-with(local-name(), '$element_name')]})]";

                    break;
                case '_order_fee_tax_code':
                    $templateOptions['pmwi_order']['fees'][0]['tax_rates'][0]['code'] = '{' . $element_name . '[1]}';
                    break;
                case '_shipping_phone':
                    $templateOptions['pmwi_order']['shipping_phone'] = '{' . $element_name . '[1]}';
                    break;
                default:
