...
jQuery(document).ready(function(){
    ...
    jQuery(document).on("keyup", ".qr-header-title, .qr-code-header-title, #channel_input_WeChat", function(){
        var qrCodeTitle = !isEmpty(jQuery("#WeChat_header_title").val())?(jQuery("#WeChat_header_title").val().toHtmlEntities()+": "):"";
        qrCodeTitle += jQuery("#channel_input_WeChat").val().toHtmlEntities();
        jQuery(".qr-code-head-title").text(qrCodeTitle);

        jQuery(".qr-code-title").text("");
        if(!isEmpty(jQuery(".qr-code-header-title").val())) {
            jQuery(".qr-code-title").text(jQuery(".qr-code-header-title").val().toHtmlEntities());
        }
    });
    ...
    jQuery(document).on("change", ".form-field-setting-col input[type='text'], .form-field-setting-col textarea, .chaty-input-text", function(){
        change_custom_preview();
    });
        ...
        if(activeChannels) {
            ...
            if(previewChannelList.length == 1 && jQuery("input[name='cta_type']:checked").val() != "chat-view") {
                ...
                var closeHtml = '<div class="csaas-channel csaas-cta-close csaas-tooltip pos-' + toolTipPosition + '" data-hover="' + + '">' +
                    ...
                    '<svg viewBox="0 0 52 52" fill="none" xmlns="http://www.w3.org/2000/svg"><ellipse cx="26" cy="26" rx="26" ry="26" fill="' + jQuery("input[name='cht_color']").val() + '"></ellipse><rect width="27.1433" height="3.89857" rx="1.94928" transform="translate(18.35 15.6599) scale(0.998038 1.00196) rotate(45)" fill="' + jQuery("input[name='cht_icon_color']").val() + '"></rect><rect width="27.1433" height="3.89857" rx="1.94928" transform="translate(37.5056 18.422) scale(0.998038 1.00196) rotate(135)" fill="' + jQuery("input[name='cht_icon_color']").val() + '"></rect></svg>' +
                    '</span>' +
                ...
                if(!isEmpty(ctaText)) {
                    ctaText = ctaText.toHtmlEntities();
                }
...
function getCTAWidgetIcon() {
    ...
    return '<span class="csaas-svg">'+getSvgIcon(widgetIcon, jQuery("input[name='cht_color']").val(), jQuery("input[name='cht_icon_color']").val())+"</span>";
...
function getChannelURL(channel, channelIcon, toolTipPosition, widgetId) {
    ...
    if(!jQuery("#chaty-social-"+channel).hasClass("has-agent-view")) {
        ...
        if(jQuery("#chaty-social-" + channel + " .chaty-title").length) {
            chaty_title = jQuery("#chaty-social-" + channel + " .chaty-title").val().toHtmlEntities();
        } else {
            if(channel == "WeChat") {
                chaty_title = channel;
            }
...
function startMakingContactForm(channel, widgetId) {
    ...
    formHtml += "<div class='csaas-contact-form-title'><div class='form-title'>"+jQuery("#cht_social_message_"+channel+"_form_title").val().toHtmlEntities()+"</div><div role='button' class='csaas-close-button'><svg width='15' height='9' viewBox='0 0 15 9' fill='none' xmlns='http://www.w3.org/2000/svg'> <path d='M1 1L7.31429 8L14 1' stroke='white' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'></path></svg></div></div>";
...
function startMakingWeChatChannel(channel, widgetId) {
    var formHtml = "";
    var widgetIndex = 0;
    var qrCodeTitle = !isEmpty(jQuery("#"+channel+"_header_title").val())?(jQuery("#"+channel+"_header_title").val().toHtmlEntities()+": "):"";
    qrCodeTitle += jQuery("#channel_input_"+channel).val().toHtmlEntities();
    ...
    if(!isEmpty(jQuery("#"+channel+"_qr_code_title").val())) {
        formHtml += '<div class="qr-code-title">'+jQuery("#"+channel+"_qr_code_title").val().toHtmlEntities()+':</div>';
    }
