...
class OhioHelper {
	...
	static function parse_acf_typo_to_css( $typo_object, $cond = false ) {
		...
		if ( $typo_object && is_object( $typo_object ) ) {
			...
			if ( isset( $typo_object->transform ) && $typo_object->transform && in_array( 'text-transform', $result_fields ) ) {
				$result_css .= 'text-transform:' . $typo_object->transform . ';';
			}
			if ( isset( $typo_object->decoration ) && $typo_object->decoration && in_array( 'text-decoration', $result_fields ) ) {
				$result_css .= 'text-decoration:' . $typo_object->decoration . ';';
			}
	...
	*/
	...
	static function get_responsive_font_css( $data, $selector = '' ) {
		...
		if ( isset( $data->tablet_size ) && !empty( $data->tablet_size ) ) {
			$result->tablet .= '@media screen and (max-width: 1180px) { ';
