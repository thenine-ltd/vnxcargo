(function ($) {
    ...
    */
    ...
    function handleStickyNav() {
        ...
        var stickyNav = $('.sticky-nav:not(.sticky-nav-product)');
        var stickyNavOffset = $(document).height() - $(window).height() - siteFooter;
        ...
        $(window).scroll(function() {
            ...
            if (!Clb.isMobile) {
                if ($(window).scrollTop() > stickyNavOffset) {
                    stickyNav.addClass('-fade-up');
                } else {
                    stickyNav.removeClass('-fade-up');
                }
            }
        });
    }
    ...
    function handleNavigations() {
        ...
        $('[aria-controls="site-menu"]').on("click", function () {
        ...
        $('[aria-controls="site-menu"]').on('click', function (e) {
            e.preventDefault();
            handleHamburgerMenu();
        });
...
jQuery(window).on('load', function() {
    ...
    function getBackgroundLines(color) {
        ...
        lines.classList.add('row-bg-lines', 'page-container', '-' + color);
        lines.innerHTML = '<div></div>'.repeat(5);
        return lines;
    }
    ...
    function prependToElementorWrapper( wrapper, childContent ) {
        var isContainer = wrapper.getAttribute( 'data-element_type' ) === 'container',
            targetElement = wrapper.querySelector( isContainer ? '.e-con-inner' : '.elementor-container' );
    ...
    function onElementorSectionUpdate( $element ) {
        ...
        if (sectionSettings.attributes['background_lines_enabled'] === 'yes') {
            prependToElementorWrapper( domElement, getBackgroundLines(sectionSettings.attributes['background_lines_style']));
        }
